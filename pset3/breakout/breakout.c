...
int main(void)
{
    ...
    while (lives > 0 && bricks > 0)
    {
        ...
        // lose if hit bottom edge
        if (getY(ball) + getHeight(ball) >= getHeight(window))
        {
            lives -= 1;
            setLocation(ball, WIDTH / 2 - RADIUS, HEIGHT / 2 + RADIUS);
            setLocation(paddle, 165, 550);
            waitForClick();
        }
...
void initBricks(GWindow window)
{
    ...
    for (int i = 0; i < COLS; i++)
    {
        ...
        {
            int xspace = BRICK_WIDTH * i + 5 * i + 2;
            int yspace = BRICK_HEIGHT * j + 5 * j + 5 * BRICK_HEIGHT;
            brick[i][j] = newGRect(xspace, yspace, BRICK_WIDTH, BRICK_HEIGHT);
            
            if (j == 0 || j == 4)
            {
                setColor(brick[i][j], "BLUE");
            }
            else if (j == 1 || j == 3)
            {
                setColor(brick[i][j], "LIGHTGRAY");
            }
            else if (j == 2)
            {
                setColor(brick[i][j], "BLACK");
            }
...
{
    // TODO
    GRect paddle = newGRect(165, 550, PADDLE_WIDTH, PADDLE_HEIGHT);
...
GLabel initScoreboard(GWindow window)
{
    ...
    setFont(scoreboard, "SansSerif-20");
    setLocation(scoreboard, 20, 20);
